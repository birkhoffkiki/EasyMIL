...
data_root_dir="/jhcnas3/Pathology/Patches/CPTAC__"$study
log_dir="eval_scripts/logs/CPTAC-"$study"_survival_"

task="CPTAC_"$study"_survival"
results=/jhcnas3/Pathology/experiments/train/split82
save_dir="/storage/Pathology/results/experiments/eval/"$task

splits_dir="splits82/"$task"_100"
...
do
    ...
    do
        ...
        exp=$model"/"$backbone
        echo "processing:"$exp
        model_exp_code="TCGA_"$train_study"_survival/"$model"/"$backbone"_s1"  # default seed is 1  

        save_exp_code=$task"/"$model"/"$backbone"_s1_512"
        # model_dir, the trained model path
        model_dir=$results"/"$model_exp_code
